...
public class MainScreen {
    ...
    public void buildMainScreen(){
        ...
        for (int r = 0; r < 9; r+=3) {
            ...
            for (int c = 0; c < 9; c+=3) {
                var endCol = c + 2;
                var spaces = getSpaceFromSector(boardService.getSpaces(), c, endCol, r, endRow);
    ...
    private List<Space> getSpaceFromSector(List<List<Space>> spaces,
    ...
    private void addFinishButton(final JPanel mainPanel) {
         finishGameButton = new FinishGameButton(e -> {
            if (boardService.gameIsFinished()){
                showMessageDialog(null, "Parábens você concluiu o jogo");
                resetGameButton.setEnabled(false);
                checkGameStatusButton.setEnabled(false);
                finishGameButton.setEnabled(false);
            } else {
                var message = "Seu jogo tem alguma inconsistência, ajuste ou tente novamente";
    ...
    private void addCheckGameStatusButton(JPanel mainPanel) {
        checkGameStatusButton = new CheckGameStatusButton(e -> {
            ...
            var message = switch (gameStatus){
                ...
                case COMPLETE -> "O jogo está complet0";
            };
